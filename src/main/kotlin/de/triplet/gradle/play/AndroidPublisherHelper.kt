...
class AndroidPublisherHelper {
    companion object {
        fun init(extension: PlayPublisherPluginExtension, config: PlayAccountConfig?): AndroidPublisher {
            ...
            if (config != null && (config.jsonFile != null || (config.serviceAccountEmail != null && config.pk12File != null))) {
        ...
        private fun authorizeWithServiceAccount(config: PlayAccountConfig?): Credential {
            if (config != null && config.serviceAccountEmail != null && config.pk12File != null) {
                return authorizeWithServiceAccount(config.serviceAccountEmail!!, config.pk12File!!)
            } else if (config != null && config.jsonFile != null) {
