    ...
    fun publishListing() {
        inputFolder?.apply {
            if (exists()) {
                ...
                listFiles(LocaleFileFilter()).forEach { updateListing(it) }

                val fileDefaultLanguage = File(inputFolder, FILE_NAME_FOR_DEFAULT_LANGUAGE)
                val defaultLanguage = fileDefaultLanguage.firstLine()
